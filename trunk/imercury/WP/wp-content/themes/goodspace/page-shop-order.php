			...
			<?php
				...
				echo '<div class="imercury-board-content">';
					...
					switch( $_POST[mode] )
					{
						case 'req':
							$today = date("Y/m/d");
							$sql = "insert into order_tbl values('','$_POST[gname]','$_POST[price]','$_POST[name_user]','$_POST[email1]','$_POST[email2]','$_POST[tel]','$_POST[zip1]','$_POST[zip2]','$_POST[addr1]','$_POST[addr2]','$_POST[contents]','$today','$_COOKIE[mid]','')";

							$db->execute($sql);
							err_move("구매신청이 정상적으로 접수되었습니다.","/");
							break;

						default:
							include( NEW_IMERCURY_DIR . '/' . $skin . '/order.php' );
							break;
					}
				echo '</div>';
				//게시판 출력용 영역 - END
